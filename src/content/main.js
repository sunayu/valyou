...
(function () {
   ...
   */
  /**
   * Portable message send. Chrome's `chrome.runtime.sendMessage(msg)` returns a
   * Promise, but Safari's `chrome.*` does NOT — it delivers the reply through a
   * callback (or via the promise-based `browser.*` API). Relying on the Promise
   * return would make `await` resolve to undefined in Safari (no settings, so no
   * filtering) and would make `.catch(...)` throw on the stats call. So we
   * always hand back a real Promise: use `browser` when present, else wrap
   * Chrome's callback form.
   *
   * @param {object} message Payload with a `type` field.
  ...
  async function fetchSettings() {
    try {
      // Ask the service worker for the current settings. We go through
      // sendMessage (above) rather than chrome.runtime.sendMessage directly so
      // this works in Safari as well as Chrome.
  ...
  function processUnit(unit) {
    ...
    // Hide/blur the whole CARD, not just the bare article: Facebook renders
    // the post header / subject line / search-result context as SIBLINGS
    // around the article, so covering only the article leaves them readable.
    // expandUnit grows to the largest wrapper still exclusive to this post.
    // Tag stays on the unit itself (a label doesn't need the extra reach).
    const target =
    ...
    // When the decision was applied to an EXPANDED wrapper, also stamp the
    // inner unit element: findUnits skips nodes carrying the processed marker,
    // so without this the next scan would re-emit the same article and count
    // it twice (the wrapper's marker isn't visible from the article).
  ...
  function report(event, category, action, kind) {
    ...
    sendMessage({ type: "stats", event, category, action, kind }).catch(() => {});
  }
