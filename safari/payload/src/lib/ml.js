...
(function (root, factory) {
  ...
  /** Lazily-decoded model cache: {weights, bias} or null when unusable. */
  let live = null;
  ...
  function load(data) {
    ...
    // Cache the decoded (float) weights plus the model's bias for scoring.
    live = { weights: decodeWeights(data), bias: data.bias };
    return true;
  }
  ...
  /**
   * Score one text: P(hateful) under the model.
   *
   * @param {string} raw Text as it appeared on the page.
   * @returns {number} Probability in (0, 1); 0 when no model is loaded.
   */
  function score(raw) {
    // No model loaded, or no features found -> no opinion; return 0.
    if (!live) return 0;
    ...
    // `z` is the running weighted sum from the primer: start at the bias, then
    // add the weight of each feature slot (scaled by `value`).
    let z = live.bias;
    for (let i = 0; i < idx.length; i += 1) z += live.weights[idx[i]] * value;

    // The sigmoid: 1 / (1 + e^-z) squashes any real number z into (0, 1) so we
    // can read it as a probability. Math.exp(-z) is e raised to the power -z.
   ...
   */
  function signalWeight(p) {
    // Below the floor the model is too unsure to say anything.
    if (p < MIN_PROB) return 0;
    // Linearly rescale p from the range [MIN_PROB, 1] onto [0, MAX_WEIGHT]:
    // (p - MIN_PROB) / (1 - MIN_PROB) is 0 at the floor and 1 at p = 1, then
    // multiplying by MAX_WEIGHT caps the model's influence at that ceiling.
    return ((p - MIN_PROB) / (1 - MIN_PROB)) * MAX_WEIGHT;
  }
