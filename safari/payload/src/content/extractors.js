...
(function (root, factory) {
  ...
  function readText(node, limit = 4000) {
    ...
    function walk(current) {
      ...
      // `children` is the list of directly nested elements. A leaf (no child
      // elements) is where the actual visible text lives, so we read it there.
      // Reading only at leaves avoids counting the same words repeatedly as we
      // would if we grabbed textContent at every level on the way down.
      const children = current.children || [];
      if (children.length === 0) {
        // `textContent` is all the text inside an element; `.trim()` drops
      ...
      // Not a leaf: recurse into each child to keep descending.
  ...
  const VIDEO_SELECTORS = [
    ...
    'div[aria-label*="video" i]', // Facebook/Instagram generic
    'div[data-video-id]',
