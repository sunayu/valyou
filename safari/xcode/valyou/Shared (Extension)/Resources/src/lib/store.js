...
(function (root, factory) {
  ...
  function area(name) {
    ...
    // Cross-browser: every caller does `await area(name).get/set(...)`, which
    // needs the promise-based storage API. Chrome's `chrome.storage` returns
    // promises in MV3, but Safari's `chrome.storage` is callback-only — its
    // promise-based equivalent lives on `browser.storage`. So prefer
    // `browser.storage[name]` when it exists (Safari, Firefox); otherwise use
    // `chrome.storage[name]` (Chrome). Without this, awaited reads resolve to
    // undefined in Safari and no settings can be loaded or decrypted.
    if (typeof browser !== "undefined" && browser.storage && browser.storage[name]) {
      return browser.storage[name];
    }
