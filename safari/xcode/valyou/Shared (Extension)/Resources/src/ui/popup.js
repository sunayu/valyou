...
(function () {
  ...
  function send(message) {
    // Cross-browser messaging. Chrome's `chrome.runtime.sendMessage(msg)`
    // returns a Promise, but Safari's `chrome.*` does NOT — it only delivers the
    // reply through a callback (or via the promise-based `browser.*` API). If we
    // relied on the Promise return, `await send(...)` would resolve to undefined
    // in Safari and the categories/stats would never load. So: use the
    // promise-based `browser` API when it exists (Safari, Firefox), otherwise
    // wrap Chrome's callback form in a Promise ourselves.
    ...
    return new Promise((resolve) => {
      try {
        chrome.runtime.sendMessage(message, (reply) => {
          // Reading lastError marks it "handled" so the browser doesn't log a
          // spurious "unchecked runtime.lastError" warning when there's no reply.
